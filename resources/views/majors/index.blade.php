...
@section('content')
    <x-page-header title="Daftar Jurusan" subtitle="Tahun Ajaran 2025/2026">
        <x-slot:action>
                    ...
                    <tr class="border-b border-[#EFEDE6] hover:bg-[#FAF9F5]">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex justify-end gap-4 text-xs font-medium">
                                <a href="{{ route('majors.show', $major['id'] ?? $major->id) }}"
                                    class="text-[#16213A] hover:text-[#A16207]">Lihat</a>
                                <a href="{{ route('majors.edit', $major['id'] ?? $major->id) }}"
                                    class="text-[#16213A] hover:text-[#A16207]">Ubah</a>
                                <form action="{{ route('majors.destroy', $major['id'] ?? $major->id) }}" method="POST"
