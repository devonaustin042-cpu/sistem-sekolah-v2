...
@section('content')
    <x-page-header title="Daftar Guru" subtitle="Tahun Ajaran 2025/2026">
        <x-slot:action>
                    ...
                    <tr class="border-b border-[#EFEDE6] hover:bg-[#FAF9F5]">
                        ...
                        <td class="px-5 py-4">
                            <div class="flex justify-end gap-4 text-xs font-medium">
                                <a href="{{ route('teachers.show', ['id' => $teacher['id'] ?? $teacher->id]) }}"
                                    class="text-[#16213A] hover:text-[#A16207]">Lihat</a>
                                <a href="{{ route('teachers.edit', ['id' => $teacher['id'] ?? $teacher->id]) }}"
                                    class="text-[#16213A] hover:text-[#A16207]">Ubah</a>
                                <form action="{{ route('teachers.destroy', ['id' => $teacher['id'] ?? $teacher->id]) }}" method="POST" onsubmit="return confirm('Hapus data guru ini dari buku induk?')">
                                    @csrf
                                    @method('DELETE')
